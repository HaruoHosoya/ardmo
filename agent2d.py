...
class Agent(nn.ModuleDict):
    ...
    def __init__(self, input_dim, state_dim, memory_dim, relation_dim, axis_dim, 
    ...
    def add_transition(self):
        ...
        if self.trans_nonlin == 'normalize':
            ...
            nl = nn.Sigmoid()
        elif self.trans_nonlin == 'relu-norm':
            nl = nn.Sequential(nn.ReLU(), Normalize())
        elif self.trans_nonlin == 'softplus-norm':
            nl = nn.Sequential(nn.Softplus(), Normalize())

        for k in range(self.axis_dim):
            self.transition_mu.append(nn.ModuleList())
            self.transition_sig.append(nn.ModuleList())
            for i in range(self.relation_dim):
                self.transition_mu[k].append(nn.Sequential(
                    nn.Linear(self.state_dim, self.state_dim),
                    nl
                    ))
                self.transition_sig[k].append(nn.Sequential(
    ...
    def transit_all(self, state, rel_axis):
        ...
        for i in range(self.relation_dim):
            next_state_mu_l.append(self.transition_mu[rel_axis][i](state))
            next_state_sig_l.append(self.transition_sig[rel_axis][i](state))
    ...
    def memory_infer(self, memory_value, inp, return_interm=False):
    ...
    def memory_retrieve(self, memory_value, state):
        ...
        elif self.state_comp == 'norm-inner':
